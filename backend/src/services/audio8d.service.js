...
export const audio8dService = {
  async startConversion(input, conversionId, isYouTube = false) {
    try {
      ...
      // Agora apenas retornamos o arquivo para processamento no frontend
      const status = {
        progress: 100,
        status: 'completed',
        file: audioPath,
      ...
      conversions.set(conversionId, errorStatus)
      throw error
    }
  },

  getProgress(conversionId) {
    const progress = conversions.get(conversionId)
